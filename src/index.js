...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/health') {
      return json({ ok: true, service: 'family-dashboard', version: '2.4.1' });
    }
    ...
    if (url.pathname === '/api/grocery') {
      ...
      if (request.method === 'GET') {
        ...
        return json({ ok: true, version: '2.4.1', items: rows.results || [] });
      }
    ...
    if (url.pathname === '/api/calendar') {
      ...
      return json({ ok: true, configured: true, version: '2.4.1', rangeStart: start.toISOString(), rangeEnd: end.toISOString(), calendarsFound: calendars.map((c) => c.name), expectedCalendars: FAMILY_CALENDARS, events });
    }
